...
RECEPTIONIST_TOOLS = [
    {
        ...
        "function": {
            ...
            "description": "Start planning, researching, and building an html presentation that responds to the user or explains what they are looking for.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
...
Your task is to first communicate with the user and determine the next step, explain, research, and/or create presentation, or ask the user for more details if it is too vague, etc. Especially, we are helping user to realize their thoughts, prototype it, build a html presentation. User is busy, so they do not want to communicate too much. You only have to ask them for more details in some specific cases:
...
from agent.tavily_search import format_web_search_context
    ...
    for i in range(len(steps_output), len(segmented_steps)):
        ...
        if i == len(segmented_steps) - 1:
            ...
            if os.path.exists(template_path):
                destination_path = os.path.join(workdir, "presentation_template.html")
                try:
                    shutil.copy2(template_path, destination_path)
                    logger.info(f"Copied slides template presentation_template.html to workspace: {destination_path}")
                except Exception as e:
                    logger.warning(f"Failed to copy slides template presentation_template.html: {e}")
            else:
                logger.warning(f"Slides template presentation_template.html not found at: {template_path}")
