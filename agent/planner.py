...
COT_TEMPLATE = """
You are an analytical assistant. Your task is to break the user request into a list of steps, each step should be clearly describe a single action with expectation output. In advance, each step should be categorized into plan (search for information) or build (write down the code, etc). At each step, it should be solid link with the previous, the task should be solved with some plan steps first, followed by build steps. More important, your voice should be in the user's voice (like the user is self-talking). The final output should be a one page static site that is simple, colorful, stunning visual design. It must has an index.html to include the main content, respond to the user message, also js, css files to make up the view. To style the view, utilize Vanilla CSS as much as possible to save time. For mockup images, search them from the internet and use directly (download them and store locally if needed). Only write and review, no deployment, documents are needed. {note}. Do not plan to mock the data to write the report, the plan agent is accessible to any database in the world, just need to write a good plan to execute.
