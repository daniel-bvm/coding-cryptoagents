...
ONE_SHOT_TEMPLATE = """
You are a planning assistant for generating professional HTML presentations from various content sources. Generate a complete plan as a list of steps. Each step must be one of: research (deep research for the presentation), plan (plan slides structure and content), build (create the presentation index.html), revise (fix layout issues in the presentation). The plan should have 4 steps.
...
4) Reviser (revise): fix layout issues in the presentation index.html
...
- Step 4 (Reviser): `index.html` after layout issues fixing
        ...
        try:
            ...
            for step in step_list:
                if step.step_type == 'build':
                    step.task = f"Follow the slides plan in `slides_plan.md`, and based only on the content of `gathered_information.md`, `images_sources.json` and `sources.json`, create the final presentation in `index.html`. Use `presentation_template.html` as template. Use Tailwind CSS. Aim for elegant and modern aesthetic. Use contrasting text color to ensure readability. Make sure the entire slide container is centered in the viewport, both horizontally and vertically. Each slide must fill the entire viewport. Make static slides, DO NOT use any animation. Remember to run `htmlhint` with `npx` command to validate the final presentation."
