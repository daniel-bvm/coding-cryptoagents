    ...
    return {
        ...
        connectEventSource() {
            ...
            this.eventSource.onerror = (error) => {
                ...
                // Show disconnection message with delay info
                const delaySeconds = Math.round(delay / 1000);
                this.showConnectionToast(
                    `Connection lost. Reconnecting in ${delaySeconds}s (attempt ${this.reconnectAttempts})`,
                    'warning'
                );
        ...
        handleRealtimeUpdate(data) {
            const { event_type, task, task_id, step, step_number, title, total_steps, plan_summary } = data.data;
            ...
            switch (event_type) {
                ...
                case 'plan_step_created':
                    this.addPlanActivity({
                        id: `step-${Date.now()}`,
                        title: `Step ${step_number}: ${step.step_type === 'plan' ? 'Research' : 'Build'}`,
                        message: step.reason,
                        details: `Task: ${title}`,
        ...
        updateStepStatus(data) {
            ...
            if (this.selectedTask && this.selectedTask.id === task_id) {
                const stepIndex = this.taskSteps.findIndex(step => step.id === step_id);
                if (stepIndex !== -1) {
        ...
        showConnectionToast(message, type = 'info') {
            ...
            toast.innerHTML = `
                ...
                ${type === 'success' ? `<button onclick="this.parentElement.remove()" class="ml-2 text-gray-400 hover:text-gray-600">
        ...
        showAdvancedToast({ title, message, type = 'info', duration = 5000, icon = null, details = null }) {
            const container = document.getElementById('toast-container');
            const toast = document.createElement('div');

            const colors = {
                'success': 'bg-gradient-to-r from-green-50 to-green-100 border-green-300 text-green-900',
                'error': 'bg-gradient-to-r from-red-50 to-red-100 border-red-300 text-red-900',
                'info': 'bg-gradient-to-r from-blue-50 to-blue-100 border-blue-300 text-blue-900',
                'warning': 'bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-300 text-yellow-900'
            };

            const iconDisplay = icon || (type === 'success' ? '✅' : type === 'error' ? '❌' : type === 'warning' ? '⚠️' : 'ℹ️');
