...
import os
from tavily_search.twitter import models
from pydantic import ValidationError
import logging
import re
import random
from tavily_search.cache.wrapper import sqlite_cache, get_cached_value, set_cache_value
...
@sqlite_cache(
    ...
    object_builder=lambda obj_dict: models.ResponseMessage[models.Tweet].model_validate(obj_dict)
)
def get_tweet_info(
    tweet_id: Union[str, int],
    twitter_api_base_url: str = TWITTER_API_URL,
    twitter_api_key: str = TWITTER_API_KEY
) -> models.ResponseMessage[models.Tweet]:
    response_model = models.ResponseMessage[models.Tweet]
    ...
    with httpx.Client(
        ...
        if str(tweet_id) not in results:
            return response_model(
                status=models.APIStatus.NOT_FOUND, 
        ...
        try:
            tweet = models.Tweet.model_validate(tweet_data)
...
def get_top_followers(
    ...
    twitter_api_base_url: str = TWITTER_API_URL,
    twitter_api_key: str = TWITTER_API_KEY
) -> models.ResponseMessage[List[models.ConnectionCard]]:
    response_model = models.ResponseMessage[List[models.ConnectionCard]]
    ...
    with httpx.Client(
        ...
        try:
            obj = [
                models.ConnectionCard.model_validate(x) 
                for x in res_json
            ]
...
@sqlite_cache(
    ...
    object_builder=lambda obj_dict: models.ResponseMessage[models.TwitterUserInfo].model_validate(obj_dict)
)
def get_twitter_user_info_by_id(
    ...
    twitter_api_base_url: str = TWITTER_API_URL, 
    twitter_api_key: str = TWITTER_API_KEY
) -> models.ResponseMessage[models.TwitterUserInfo]:
    response_model = models.ResponseMessage[models.TwitterUserInfo]
    ...
    with httpx.Client(
        ...
        try:
            obj = models.TwitterUserInfo.model_validate(res_json['result'])
...
def get_twitter_user_info_by_username(
    ...
    with httpx.Client(
        ...
        try:
            user = models.TwitterUserInfo.model_validate(user_info)
...
@sqlite_cache(
    ...
    object_builder=lambda obj_dict: models.ResponseMessage[models.TweetPage].model_validate(obj_dict)
)
def list_tweets_of_user(
    ...
    twitter_api_base_url: str = TWITTER_API_URL,
    twitter_api_key: str = TWITTER_API_KEY
) -> models.ResponseMessage[models.TweetPage]:
    response_model = models.ResponseMessage[models.TweetPage]
    ...
    with httpx.Client(
        ...
        try:
            obj = models.TweetPage.model_validate(res_json)
            return response_model(result=obj)
...
is_valid_user = lambda user: (
    isinstance(user, models.TwitterUserInfo) 
    and user.id 
    and user.username
)
...
def get_tweet_threads_by_id(
    ...
    twitter_api_base_url: str = TWITTER_API_URL,
    twitter_api_key: str = TWITTER_API_KEY,
) -> models.ResponseMessage[dict[str, list[models.Tweet]]]:

    response_model = models.ResponseMessage[dict[str, list[models.Tweet]]]
    ...
    threads: dict[str, list[models.Tweet]] = {}
...
@sqlite_cache(
    ...
    object_builder=lambda obj_dict: models.ResponseMessage[models.TwitterSearchResult].model_validate(obj_dict)
)
def search_twitter_news(
    ...
    use_raw=False,
    no_duplication=True,
) -> models.ResponseMessage[models.TwitterSearchResult]:
    response_model = models.ResponseMessage[models.TwitterSearchResult]
    if not use_raw:
    ...
    search_result = models.TwitterSearchResult.model_validate(resp_json["result"])
    filtered_search_result = models.TwitterSearchResult(
        LookUps={},
...
def get_mentioned_tweets(
    ...
    with httpx.Client(
        ...
        try:
            tweet_page = models.TweetPage.model_validate(resp_json["result"])
        except ValidationError as e:
            logger.error(
        ...
        for tweet in tweet_page.data:
            ...
            if user_info_response.status != models.APIStatus.OK:
                continue
        ...
        filtered_tweet_page = models.TweetPage(
